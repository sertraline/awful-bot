...
class VkAudio(object):
    ...
    __slots__ = ('_vk', 'user_id', 'convert_m3u8_links', 'debug')
    ...
    def __init__(self, vk, debugger, convert_m3u8_links=True):
    ...
    def get_iter(self, owner_id=None, album_id=None, access_hash=None, count=None):
        ...
        while True:
            ...
            self.debug(f'Album id: {album_id}')
            tracks = scrap_data(
    ...
    async def get(self, owner_id=None, album_id=None, access_hash=None, count=None):
        ...
        audio_list = list(self.get_iter(owner_id, album_id, access_hash, count))
        return audio_list
    ...
    def get_albums_iter(self, owner_id=None):
        ...
        while True:
            ...
            albums = scrap_albums(response.content.decode('utf-8'))
...
def scrap_data(html, user_id, filter_root_el=None, convert_m3u8_links=True, http=None):
    ...
    key = next(iter(filter_root_el.keys()))
    
    if js and filter_root_el[key] != 'audioPlaylist__list':
        try:
            print("Iterating", filter_root_el)
            for key, val in js['data'][0].items():
                data_audio = val[1]
                if int(data_audio[21]) == 1:
                    print(f"Content blocked: {data_audio[3]}")
                    continue
                ...
                if len(data_audio[13]) == 6:
                    ...
                    try:
                        data_audio[13] = [data_audio[13][1], data_audio[13][3]]
                    except IndexError:
                        print(f"Content blocked: {data_audio[3]}")
                        continue
        ...
        html_str = None
        if js and filter_root_el[key] == 'audioPlaylist__list':
            html_str = [str(html_part) for html_part in js['data']]
        
        if not html_str:
            playlist_snippets = soup.find_all('div',
                                    {'class': "audioPlaylistSnippet__list"})
            for playlist in playlist_snippets:
                playlist.decompose()
        else:
            html_str = ' '.join(html_str)
            root_el = BeautifulSoup(html_str, features='html5lib')
    ...
    for ids_group in [ids[i:i + 3] for i in range(0, len(ids), 3)]:
        ...
        try:
            ...
            print(','.join(['_'.join(i) for i in ids_group]))
